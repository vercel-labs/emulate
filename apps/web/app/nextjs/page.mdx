...
Some services (Stripe, GitHub) emit webhook events. To deliver them to a route on the same origin, register the URL via the `webhooks` field. Relative URLs are resolved against the running origin, so the same config works on localhost and Vercel preview deployments.
...
Webhook deliveries are awaited before the dispatching request returns, so by the time a redirect (e.g. from `/checkout/:id/complete` to your `success_url`) reaches the browser, the webhook handler has already run. No `stripe listen` or ngrok needed.

See [Stripe Checkout](/examples/stripe-checkout) for an end-to-end example.
