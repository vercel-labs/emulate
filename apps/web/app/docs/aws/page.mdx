...
- `CreateIntegration` / `GetIntegration` / `GetIntegrations` / `DeleteIntegration` - `AWS_PROXY` Lambda integrations with payload format version `2.0`
- `CreateRoute` / `GetRoute` / `GetRoutes` / `DeleteRoute` - exact HTTP routes, path parameter routes, `ANY` routes, greedy proxy routes, and `$default`
