...
Explicit keys are excluded. Persistence restores the same generated identity across cold starts. The persisted snapshot contains the private key, so use a private backend and never expose generated secrets through an event handler or public runtime config.
