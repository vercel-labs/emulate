...
Custom definitions created with `defineEmulator` from `emulate` work in the CLI, in-process tests, and both framework adapters. The same definition provides typed state, HTTP handlers, reset, versioned snapshots, and optional inspection/persistence. See [Custom HTTP APIs](/docs/custom-apis) for the quickstart, API reference, and complete lifecycle semantics.
