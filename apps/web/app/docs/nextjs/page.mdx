...
Custom definitions created with `defineEmulator` from `emulate` work in the CLI, in-process tests, and both framework adapters. The same definition provides typed state, HTTP handlers, reset, versioned snapshots, and optional inspection/persistence. Export `OPTIONS` so Next.js forwards preflight requests and custom OPTIONS routes. Root-relative redirect locations stay under the mounted service path, while custom HTML bodies pass through unchanged. See [Custom emulators](/docs/custom-emulators) for the quickstart, API reference, and complete lifecycle semantics.
