...
GitHub App seeds may omit `private_key`. Keep the returned object in this server route to access generated keys without adding an HTTP endpoint:
...
Explicit keys are excluded. Persistence restores the same generated identity across cold starts. The persisted snapshot contains the private key, so use a private backend and never return generated secrets from a route or import the route module into Client Components.
