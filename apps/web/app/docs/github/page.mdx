...
Issue GraphQL reads expose `Issue.parent`, opaque cursor-paginated `subIssues` and `blockedBy` connections. The `addSubIssue` and `addBlockedBy` mutations share relationship state with REST, enforce equivalent permissions and validation, and echo `clientMutationId` exactly.
...
- `PATCH /repos/:owner/:repo/issues/:number/sub_issues/priority` - move a child with `after_id` or `before_id`
- `GET/POST /repos/:owner/:repo/issues/:number/dependencies/blocked_by` - list or add blocking issues
...
Relationship lists use `page` and `per_page`, cap `per_page` at 100, and return `Link` headers when more pages are available. Sub-issues have one parent and explicit sibling order. The `replace_parent` write option moves an existing child atomically. Hierarchy and dependency edges are independent, and self-references, duplicates, and cycles are rejected without changing state. Sub-issues must be in repositories owned by the same account. Dependency targets may be cross-repository when the caller can read both repositories. Writes require issue write access on the route repository and issue read access to referenced repositories.
