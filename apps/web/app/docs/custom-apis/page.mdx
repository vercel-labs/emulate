...
`init` prints the generated test command. `start` prints the service URL, inspector URL, and a direct Routes link for each custom service. Use the printed service URL for requests because an existing config may assign a different port. Requests, Routes, and State are available in the inspector, normally at `http://localhost:4000/_emulate` for a new config. Reset restores the captured initial seed.
