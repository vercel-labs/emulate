...
The scaffold creates `emulators/inventory.ts` and a Node test, then adds a service entry to a discovered YAML, JSON, TypeScript, or JavaScript config. If the executable config uses an unusual default export, follow the manual import and service entry printed by the command. The sample implements reservations that consume stock, cancellation that returns stock, and an out-of-stock response.
...
`init` prints the generated test command. `start` prints the inspector URL and a direct Routes link for each custom service. Requests, Routes, and State are available in the inspector, normally at `http://localhost:4000/_emulate`. Reset restores the captured initial seed.
