...
Files and packages resolve from the config project, including when using `npx`. Local `.ts`, `.mts`, `.js`, and `.mjs` modules need no build step. The CLI uses Node 24's native TypeScript transform and module hooks, with no additional runtime dependencies. TypeScript `paths` and `baseUrl` aliases work, including JSONC configs and inherited `extends`. JSX and compiler plugins are not supported. Use erasable TypeScript for definitions that also run directly in Node tests. Installed packages must export a built JavaScript definition as their default export.
