...
Files and packages resolve from the config project, including when using `npx`. Local `.ts`, `.mts`, `.js`, and `.mjs` modules need no build step. TypeScript path aliases work. Installed packages must export a built JavaScript definition as their default export.
