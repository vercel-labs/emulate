...
recipient lists, content, custom headers, and base64 attachments. Tests using
`createServer` can inspect them with
`store.collection("twilio.sendgrid.emails").all()`.

Library users can configure keys with
`createTwilioPlugin({ sendgrid: { apiKeys } })`.
