...
`chats[].type` supports `private` and `group`. Supergroups, channels, and forum topics are created at runtime via the control plane (`createSupergroup`, `createChannel`, `createForumTopic`).
