 ...
 *   - the Telegram emulator (TELEGRAM_API_ROOT=http://localhost:4014)
...
export function registerHandlers(bot: Bot): void {
  ...
  bot.command("stream", async (ctx) => {
    ...
    const chunks = [
      ...
      "Thinking..",
      "Thinking...",
      "Here is your plan: Tangier → Chefchaouen → Fez.",
    ];
  ...
  bot.callbackQuery(/^opt:/, async (ctx) => {
    ...
    const label = choice === "a" ? "You picked A." : choice === "b" ? "You picked B." : "Cancelled.";
