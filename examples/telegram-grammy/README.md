...
```bash
# Terminal 1
npx emulate --service telegram --port 4014
...
```bash
curl -X POST http://localhost:4014/_emu/telegram/chats/1001/messages \
  -H 'content-type: application/json' \
  -d '{"userId":1001,"text":"/start"}'

curl -s 'http://localhost:4014/_emu/telegram/chats/1001/messages?scope=bot'
```

Or open `http://localhost:4014/` in a browser — the inspector shows chats, messages, and the Update queue live.
