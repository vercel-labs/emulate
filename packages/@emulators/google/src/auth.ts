...
const GMAIL_SETTINGS_BASIC_SCOPES = [GMAIL, GMAIL_SETTINGS_BASIC];
const GMAIL_SETTINGS_SHARING_SCOPES = [GMAIL, GMAIL_SETTINGS_SHARING];
...
const DRIVE_CONTENT_WRITE_SCOPES = [DRIVE, DRIVE_FILE];
...
export function driveScopes(c: Context): string[] {
  ...
  if (c.req.method === "PUT") {
    return DRIVE_CONTENT_WRITE_SCOPES;
  }
...
export function gmailScopes(c: Context): string[] {
  ...
  if (path.includes("/settings/filters")) {
    return GMAIL_SETTINGS_BASIC_SCOPES;
  }
  if (path.includes("/settings/forwardingAddresses")) {
    return GMAIL_SETTINGS_BASIC_SCOPES;
  }
  if (path.includes("/settings/sendAs")) {
    return [...GMAIL_SETTINGS_BASIC_SCOPES, ...GMAIL_SETTINGS_SHARING_SCOPES];
  }
