...
export function buildSamlResponse(params: SamlResponseParams): string {
  ...
  const attributeStatements = attributes.length > 0
    ? `<saml2:AttributeStatement>${attributes.map(a =>
        `<saml2:Attribute Name="${escapeXml(a.name)}" NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"><saml2:AttributeValue xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xsi:type="xs:string">${escapeXml(a.value)}</saml2:AttributeValue></saml2:Attribute>`
      ).join("")}</saml2:AttributeStatement>`
    : "";

  return `<saml2p:Response xmlns:saml2p="${SAML_NS.PROTOCOL}" Destination="${escapeXml(destination)}" ID="${escapeXml(responseId)}" InResponseTo="${escapeXml(inResponseTo)}" IssueInstant="${escapeXml(now)}" Version="2.0"><saml2:Issuer xmlns:saml2="${SAML_NS.ASSERTION}">${escapeXml(issuer)}</saml2:Issuer><saml2p:Status><saml2p:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Success"/></saml2p:Status><saml2:Assertion xmlns:saml2="${SAML_NS.ASSERTION}" ID="${escapeXml(assertionId)}" IssueInstant="${escapeXml(now)}" Version="2.0"><saml2:Issuer>${escapeXml(issuer)}</saml2:Issuer><saml2:Subject><saml2:NameID Format="${escapeXml(nameIdFormat)}">${escapeXml(nameId)}</saml2:NameID><saml2:SubjectConfirmation Method="urn:oasis:names:tc:SAML:2.0:cm:bearer"><saml2:SubjectConfirmationData InResponseTo="${escapeXml(inResponseTo)}" NotOnOrAfter="${escapeXml(notOnOrAfter)}" Recipient="${escapeXml(destination)}"/></saml2:SubjectConfirmation></saml2:Subject><saml2:Conditions NotBefore="${escapeXml(notBefore)}" NotOnOrAfter="${escapeXml(notOnOrAfter)}"><saml2:AudienceRestriction><saml2:Audience>${escapeXml(audienceEntityId)}</saml2:Audience></saml2:AudienceRestriction></saml2:Conditions><saml2:AuthnStatement AuthnInstant="${escapeXml(authnInstant)}" SessionIndex="${escapeXml(assertionId)}" SessionNotOnOrAfter="${escapeXml(sessionNotOnOrAfter)}"><saml2:AuthnContext><saml2:AuthnContextClassRef>urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport</saml2:AuthnContextClassRef></saml2:AuthnContext></saml2:AuthnStatement>${attributeStatements}</saml2:Assertion></saml2p:Response>`;
}
