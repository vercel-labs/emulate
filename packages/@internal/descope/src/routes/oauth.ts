...
import type { DescopeUser } from "../entities.js";

// NOTE: If tests fail, try descope-specific format
const JWT_SECRET = new TextEncoder().encode("emulate-descope-jwt-secret");
...
function getPendingCodes(store: Store): Map<string, PendingCode> {
  let map = store.getData<Map<string, PendingCode>>("descope.oauth.pendingCodes");
  if (!map) {
    map = new Map();
    store.setData("descope.oauth.pendingCodes", map);
  }
  return map;
}
...
export function oauthRoutes({ app, store, baseUrl, tokenMap }: RouteContext): void {
  ...
  app.get("/.well-known/openid-configuration", (c) => {
    return c.json({
      ...
      jwks_uri: `${baseUrl}/oauth2/v1/apps/certs`,
  ...
  app.get("/oauth2/v1/apps/certs", (c) => {
    return c.json({ keys: [] });
  });

  // ---------- Authorization page (with dynamic projectId) ----------

  app.get("/oauth2/v1/apps/:projectId/authorize", (c) => {
    ...
    const clientsConfigured = ds.oauthClients.all().length > 0;
    let clientName = "";
    if (clientsConfigured) {
      const client = ds.oauthClients.findOneBy("client_id", client_id);
      if (!client) {
        return c.html(
    ...
    const subtitleText = clientName
      ? `Sign in to <strong>${escapeHtml(clientName)}</strong> with your Descope account.`
    ...
    const userButtons = users
      .map((user) => {
        return renderUserButton({
          ...
          name: user.name,
          email: user.email,
          formAction: `/oauth2/v1/apps/${projectId}/authorize/callback`,
          hiddenFields: {
            email: user.email,
            redirect_uri,
            scope,
  ...
  app.post("/oauth2/v1/apps/:projectId/authorize/callback", async (c) => {
    const projectId = c.req.param("projectId");
    ...
    debug("descope.oauth", `[Descope callback] projectId=${projectId} code=${code.slice(0, 8)}... email=${email}`);
  ...
  app.post("/oauth2/v1/apps/token", async (c) => {
    ...
    const clientsConfigured = ds.oauthClients.all().length > 0;
    if (clientsConfigured) {
      const client = ds.oauthClients.findOneBy("client_id", bodyClientId);
      if (!client) {
    ...
    const user = ds.users.findOneBy("email", pending.email as DescopeUser["email"]);
    if (!user) {
      return c.json({ error: "invalid_grant", error_description: "User not found." }, 400);
    }

    const accessToken = "descope_" + randomBytes(20).toString("base64url");
    ...
    debug("descope.oauth", `[Descope token] issued token for ${user.email}`);
  ...
  app.get("/oauth2/v1/apps/userinfo", (c) => {
    const authUser = c.get("authUser");
    if (!authUser) {
      return c.json({ error: "invalid_token", error_description: "Authentication required." }, 401);
    }

    const user = ds.users.findOneBy("email", authUser.login as DescopeUser["email"]);
    if (!user) {
  ...
  app.post("/oauth2/v1/apps/revoke", async (c) => {
    const contentType = c.req.header("Content-Type") ?? "";
    const rawText = await c.req.text();
