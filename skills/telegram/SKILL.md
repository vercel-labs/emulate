...
```bash
# Telegram only — default port 4014
...
const tg = await createEmulator({ service: "telegram", port: 4014 });
// tg.url === 'http://localhost:4014'
```
...
const tg = createTelegramTestClient("http://localhost:4014");
...
Set `TELEGRAM_API_ROOT=http://localhost:4014` in tests.
...
Open `http://localhost:4014/` in a browser for a read-only view of bots, chats, messages, and the Update queue. Useful for debugging test failures.
